...
class TimeSystem(ISystem, ICheckpointedSystem):
    """
    ...
    runtime_state_contract = {
        ...
        "last_event_total_minutes": SYSTEM_STATE_CHECKPOINT,
    }

    # Pre-calculated "Game Minutes per Real Second" per speed level.
    # This lives on the class so save/load audits only track truly mutable runtime state.
    MINUTES_PER_SEC: Dict[int, float] = {
        1: 30.0,
        2: 60.0,
        3: 120.0,
        4: 600.0,
    ...
    def export_persistent_state(self) -> dict[str, Any]:
        return export_declared_checkpoint_state(self)

    def import_persistent_state(self, data: Mapping[str, Any]) -> None:
        self.real_sec_timer = float(data.get("real_sec_timer", 0.0) or 0.0)
        self.last_event_total_minutes = int(data.get("last_event_total_minutes", 0) or 0)
