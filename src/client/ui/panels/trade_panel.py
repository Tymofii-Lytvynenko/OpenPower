...
class TradePanel:
    """
    Diplomatic trade panel for territory and technology deals.

    This panel must not use resource-market tables such as trade_network. Resource
    flows belong to ResourcesPanel; TRADE here means negotiated exchanges between
    countries, e.g. territory or technology transfer.
    """
    ...
    def _render_content(self, state, target_tag: str, selected_region_id, is_own: bool):
        ...
        imgui.text_wrapped(
            ...
            "and territory transfer. Resource imports/exports are shown in the "
            "Resources panel and are not part of this interface."
        )
        imgui.dummy((0, 8))
        ...
        partner_tag, partner_label = partners[self.partner_idx]

        imgui.text_disabled("PARTNER")
        imgui.same_line(140)
        imgui.set_next_item_width(280)
        changed, self.partner_idx = imgui.combo("##trade_partner", self.partner_idx, [label for _, label in partners])
        if changed:
            self.offer_region_idx = 0
            self.request_region_idx = 0
            self.offer_tech_idx = 0
        ...
        imgui.same_line(140)
        imgui.set_next_item_width(280)
        imgui.combo("##trade_deal_type", self.deal_type_idx, self.DEAL_TYPES)
        imgui.dummy((0, 8))
    ...
    def _render_technology_trade(self, state, target_tag: str, partner_tag: str):
        ...
        if not tech_tables:
            ...
            imgui.text_wrapped(
                ...
                "inventory/research table and a diplomacy proposal action."
            )
            return
    ...
    def _regions_for_owner(self, state, owner_tag: str) -> list[tuple[int, str]]:
        ...
        try:
            ...
            rows = regions.filter(pl.col("owner") == owner_tag).iter_rows(named=True)
            result = []
            for row in rows:
    ...
    def _technology_tables(self, state) -> list[str]:
        return sorted(
            name for name in state.tables.keys()
            if "tech" in name.lower() or "research" in name.lower()
        )
